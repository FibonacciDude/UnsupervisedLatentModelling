...
for seed in seeds:
    seed = str(seed)
    #for t in ['intrinsic', 'extrinsic']: # this before...
    for t in ['intrinsic']:
        for what in ['mean', 'std']:
            val = (np.load("data/" + seed + "_" + t + "_" + "int" + "_" + what + ".npy"))
            index.append(("Rewards in Pendulum Dynamics", seed, t, what))
            vals.append(val)
                #vals.append(val[col-1])


path = "data/ScienceFairDynamicsExploitData.xlsx"
index = pd.MultiIndex.from_tuples(index)
data = np.array(vals)
pop = pd.DataFrame(data, index=index, columns=[list(range(10))]) # 20 before...
print(pop)
